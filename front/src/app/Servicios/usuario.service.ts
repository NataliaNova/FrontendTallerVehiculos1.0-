...
export class UsuarioService {
  ...
  crear(data: any) {
    return this._htpp.post<any>(base_url + 'usuario/crearUsuario', data).pipe(
      map((res: any) => {
        return res;
      })
    );
  ...
  login(user: any, obtenerToken = null): Observable<any> {
    ...
    return this._htpp.post(base_url + 'usuario/login', json, {
      headers: headers,
    });
  }
  ...
  obtenerUsuarios() {
    return this._htpp.get<any>(base_url + 'usuario/listar').pipe(
      map((res: any) => {
        return res;
      })
    );
  ...
  obtenerUsuario(id: number) {
    return this._htpp.get<any>(base_url + 'usuario/listarUsuario/' + id).pipe(
      map((res: any) => {
        return res;
      })
    );
